...
import { findOpponents, loadCatalogSnapshot, loadPlayerProfile, loadProgress, loadSession, loadStats, loadWallet, signIn as signInRemote, signOut as signOutRemote, signUp as signUpRemote, startBattle as startBattleRemote, type BattleResult, type Opponent, type PlayerProfile, type PlayerProgress, type PublicCard, type Session, type Wallet } from '@/lib/supabase';
...
export function GameProvider({ children }: { children: React.ReactNode }) {
    ...
    const refresh = async () => {
      ...
      try {
        ...
        } else { setPlayer(null); setProgress(null); setWallet(null); setStats(null); }
    ...
    const signOut = async () => { await signOutRemote(); setSession(null); setPlayer(null); setProgress(null); setWallet(null); setStats(null); setOpponents([]); };
    ...
    const value = useMemo(() => ({ session, player, progress, wallet, stats, cardsTotal, featuredCards, syncState, authLoading, authError, signIn, signUp, signOut, refresh, opponents, battleLoading, battleResult, findOpponents: find, startBattle: battle, clearBattleResult: () => setBattleResult(null) }), [session, player, progress, wallet, stats, cardsTotal, featuredCards, syncState, authLoading, authError, opponents, battleLoading, battleResult]);
