...
function EmptyState({ icon, title, copy, colors }: { icon: keyof typeof Feather.glyphMap; title: string; copy: string; colors: Colors }) {
  return <View style={[styles.empty, { backgroundColor: colors.panel, borderColor: colors.border }]}><Feather name={icon} size={26} color={colors.mutedForeground} /><Text style={[styles.emptyTitle, { color: colors.foreground }]}>{title}</Text><Text style={[styles.emptyCopy, { color: colors.mutedForeground }]}>{copy}</Text></View>;
}
...
export default function WorldScreen() {
  ...
  return <ScreenShell surface="world"><ScrollView style={styles.screen} contentContainerStyle={{ paddingTop: insets.top + 18, paddingBottom: insets.bottom + 108 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { void load(true); }} tintColor={colors.accent} />} showsVerticalScrollIndicator={false}>
    ...
    <View style={styles.content}>
      {loading && !snapshot ? <View style={styles.loading}><ActivityIndicator size="large" color={colors.accent} /><Text style={[styles.copy, { color: colors.mutedForeground }]}>Abriendo rutas del mundo…</Text></View> : null}
      {error && !snapshot ? <><View style={[styles.error, { borderColor: colors.danger, backgroundColor: `${colors.danger}12` }]}><Feather name="alert-circle" size={18} color={colors.danger} /><Text style={[styles.errorText, { color: colors.foreground }]}>{error}</Text></View><Pressable testID="world-retry" accessibilityRole="button" onPress={() => { void load(); }} style={[styles.primaryButton, { backgroundColor: colors.accent }]}><Text style={[styles.primaryButtonText, { color: colors.ink }]}>REINTENTAR SINCRONIZACIÓN</Text></Pressable></> : null}
