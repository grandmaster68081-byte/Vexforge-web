...
function formatDate(value: string | null | undefined) {
  ...
  return Number.isNaN(date.getTime()) ? 'FECHA NO REPORTADA' : date.toLocaleDateString('es-ES', { day: 'numeric', month: 'short', year: 'numeric' }).replace('.', '');
}
...
function signalLabel(value: string | null | undefined, missingLabel: string) {
  const normalized = value?.trim();
  return normalized || missingLabel;
}

function cosmeticTypeLabel(value: string | null | undefined) {
  return signalLabel(value?.replace(/_/g, ' '), 'TIPO NO REPORTADO');
}

function effectLabel(effectType: string | null | undefined, effectValue: number | null | undefined) {
  const label = signalLabel(effectType, 'EFECTO NO REPORTADO');
  const value = typeof effectValue === 'number' && Number.isFinite(effectValue) ? ` · ${effectValue}` : '';
  return `${label}${value}`;
}

function nftStatusLabel(value: string | null | undefined) {
  const normalized = value?.trim().toLowerCase();
  if (!normalized) return 'ESTADO NO REPORTADO';
  ...
  return known[normalized] ?? `ESTADO: ${value?.trim()}`;
}

function nftCardLabel(value: string | null | undefined) {
  const normalized = value?.trim();
  return normalized ? `CARTA ${normalized.slice(0, 8)}` : 'CARTA NO REPORTADA';
}
