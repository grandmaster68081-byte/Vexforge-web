...
export type MobileSocialFriend = { id: string; friend_id: string; display_name: string | null; level: number | null; created_at: string };
...
export type MobileClan = { id: string; code: string | null; name: string | null; description: string | null; leader_player_id: string | null; prestige: number | null; contribution_total: number | null; created_at: string | null; rank_position?: number | null };
export type MobileClanMember = { id: string; player_id: string; role: string | null; joined_at: string | null; contribution_accumulated: number | null; display_name: string | null };
export type MobileClanWar = { id: string; clan_a_id: string; clan_b_id: string; status: string | null; created_at: string; opponent_name: string | null };
export type MobileSocialMatch = { id: string; player_a: string; player_b: string; winner: string | null; status: string | null; elo_change_a: number | null; elo_change_b: number | null; created_at: string; opponent_name: string | null };
export type MobilePvpRanking = { player_id: string; display_name: string | null; mmr: number | null; wins: number | null; losses: number | null; draws: number | null; rank_position: number | null };
...
export async function loadSocialSnapshot(session: Session, playerId: string): Promise<MobileSocialSnapshot> {
  ...
  const mapSocialLevel = (value: number | null | undefined) => typeof value === 'number' && Number.isFinite(value) ? value : null;
  const friends = relationshipRows.filter((row) => row.status === 'accepted').map((row) => { const friendId = row.player_a_id === playerId ? row.player_b_id : row.player_a_id; const info = relationshipNames.get(friendId); return { id: row.id, friend_id: friendId, display_name: info?.display_name ?? null, level: mapSocialLevel(info?.level), created_at: row.created_at }; });
  const pendingRequests = relationshipRows.filter((row) => row.status === 'pending' && row.initiated_by !== playerId).map((row) => { const friendId = row.initiated_by; const info = relationshipNames.get(friendId); return { id: row.id, friend_id: friendId, display_name: info?.display_name ?? null, level: mapSocialLevel(info?.level), created_at: row.created_at }; });
  ...
  if (memberRows[0]) {
    ...
    if (clanRow) {
      ...
      const mapSocialNumber = (value: unknown) => typeof value === 'number' && Number.isFinite(value) ? value : null;
      const mobileMembers = rosterRows.map((row) => ({ id: row.id, player_id: row.player_id, role: typeof row.role === 'string' && row.role.trim() ? row.role : null, joined_at: row.joined_at, contribution_accumulated: mapSocialNumber(row.contribution_accumulated), display_name: rosterNames.get(row.player_id)?.display_name ?? null }));
      const metadata = clanRow.metadata as Record<string, unknown> | null;
      clan = { clan: { id: String(clanRow.id), code: typeof clanRow.code === 'string' && clanRow.code.trim() ? clanRow.code : null, name: typeof clanRow.name === 'string' && clanRow.name.trim() ? clanRow.name : null, description: typeof metadata?.description === 'string' && metadata.description.trim() ? metadata.description : null, leader_player_id: typeof clanRow.leader_player_id === 'string' ? clanRow.leader_player_id : null, prestige: mapSocialNumber(clanRow.prestige), contribution_total: mapSocialNumber(clanRow.contribution_total), created_at: typeof clanRow.created_at === 'string' ? clanRow.created_at : null }, member: { id: membership.id, player_id: membership.player_id, role: typeof membership.role === 'string' && membership.role.trim() ? membership.role : null, joined_at: membership.joined_at, contribution_accumulated: mapSocialNumber(membership.contribution_accumulated), display_name: rosterNames.get(playerId)?.display_name ?? null }, members: mobileMembers, wars: warRows.map((row) => ({ ...row, status: typeof row.status === 'string' && row.status.trim() ? row.status : null, opponent_name: warNames.get(row.clan_a_id === membership.clan_id ? row.clan_b_id : row.clan_a_id) ?? null })) };
    }
  }
  const mappedClans = clansRows.map((row, index) => { const metadata = row.metadata as Record<string, unknown> | null; return { id: String(row.id), code: typeof row.code === 'string' && row.code.trim() ? row.code : null, name: typeof row.name === 'string' && row.name.trim() ? row.name : null, description: typeof metadata?.description === 'string' && metadata.description.trim() ? metadata.description : null, leader_player_id: typeof row.leader_player_id === 'string' ? row.leader_player_id : null, prestige: typeof row.prestige === 'number' && Number.isFinite(row.prestige) ? row.prestige : null, contribution_total: typeof row.contribution_total === 'number' && Number.isFinite(row.contribution_total) ? row.contribution_total : null, created_at: typeof row.created_at === 'string' ? row.created_at : null, rank_position: index + 1 }; });
  const matchNames = await resolveMobilePublicNames(session, matchRows.flatMap((row) => [String(row.player_a ?? ''), String(row.player_b ?? '')]));
  const matches = matchRows.map((row) => { const playerA = String(row.player_a ?? ''); const playerB = String(row.player_b ?? ''); const opponentId = playerA === playerId ? playerB : playerA; const toFiniteNumber = (value: unknown) => typeof value === 'number' && Number.isFinite(value) ? value : null; return { id: String(row.id), player_a: playerA, player_b: playerB, winner: typeof row.winner === 'string' ? row.winner : null, status: typeof row.status === 'string' && row.status.trim() ? row.status : null, elo_change_a: toFiniteNumber(row.elo_change_a), elo_change_b: toFiniteNumber(row.elo_change_b), created_at: typeof row.created_at === 'string' ? row.created_at : '', opponent_name: matchNames.get(opponentId)?.display_name ?? null }; });
  return { friends, pendingRequests, challenges, clan, clans: mappedClans, rankings: (rankingRows ?? []).map((row) => ({ player_id: String(row.player_id), display_name: typeof row.display_name === 'string' ? row.display_name : null, mmr: typeof row.mmr === 'number' && Number.isFinite(row.mmr) ? row.mmr : null, wins: typeof row.wins === 'number' && Number.isFinite(row.wins) ? row.wins : null, losses: typeof row.losses === 'number' && Number.isFinite(row.losses) ? row.losses : null, draws: typeof row.draws === 'number' && Number.isFinite(row.draws) ? row.draws : null, rank_position: typeof row.rank_position === 'number' && Number.isFinite(row.rank_position) ? row.rank_position : null })), matches, seasonName: season?.name ?? season?.season_key ?? null };
}
