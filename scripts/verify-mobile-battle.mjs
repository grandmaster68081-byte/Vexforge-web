...
const assertions = [
  ...
  ['battlefield activates the approved scene as a native viewport', contents.battlefield.includes('BattlefieldScene') && contents.battlefield.includes('battlefield-scene-viewport') && contents.battlefield.includes('CANONICAL_BACKGROUNDS.pvp') && contents.battlefield.includes('sceneContent') && contents.battlefield.includes('LinearGradient')],
  ['battlefield keeps hand and command states honest', contents.battlefield.includes('>MANO</Text>') && contents.battlefield.includes('>COMMAND</Text>') && contents.battlefield.includes('NO REPORTADO')],
  ['battle screen owns the safe-area contract', contents.screen.includes('useSafeAreaInsets') && contents.screen.includes('paddingTop: insets.top + 20') && contents.screen.includes('paddingBottom: insets.bottom + 108') && contents.screen.includes('paddingLeft: Math.max(18, insets.left)') && contents.screen.includes('paddingRight: Math.max(18, insets.right)') && contents.screen.includes('contentInsetAdjustmentBehavior="never"') && contents.screen.includes('scrollIndicatorInsets')],
  ['battlefield parallax is bounded and native-driven', contents.screen.includes('Animated.ScrollView') && contents.screen.includes('Animated.event') && contents.screen.includes('useNativeDriver: true') && contents.screen.includes('parallaxY={battlefieldScrollY}') && contents.battlefield.includes('parallaxMaxTranslateY') && contents.battlefield.includes('extrapolate: \'clamp\'') && contents.battlefield.includes('const parallaxTransform = reducedMotion')],
  ['battlefield animation keeps a small performance budget', contents.battlefield.includes('useNativeDriver: true') && contents.battlefield.includes('Animated.loop') && contents.battlefield.includes('maxAnimatedSceneLayers') && contents.battlefield.includes('sceneGlowCyan') && contents.battlefield.includes('sceneGlowEmber')],
  ['battlefield keeps critical labels legible over scene art', contents.battlefield.includes('sceneBackdrop') && contents.battlefield.includes('LinearGradient') && contents.battlefield.includes('backgroundColor: `${colors.panelStrong}F2`') && contents.battlefield.includes('numberOfLines={2}') && contents.battlefield.includes('numberOfLines={1}')],
  ...
  ['battlefield does not draw absent HP or turn as confirmed values', contents.battlefield.includes('if (typeof hp !== \'number\'') && contents.battlefield.includes('max <= 0) return null') && contents.battlefield.includes('percent !== null') && contents.battlefield.includes('NO REPORTADO') && !contents.battlefield.includes(": '—'")],
   ['battlefield keeps empty slot and reserve signals explicit', contents.battlefield.includes('POSICIÓN VACÍA') && contents.battlefield.includes('RESERVA VACÍA') && contents.battlefield.includes('UNIDAD NO REPORTADA') && !contents.battlefield.includes('Posición vacía') && !contents.battlefield.includes('ESPERANDO UNIDAD') && !contents.battlefield.includes('>VACÍA<')],
  ...
  ['turn detail keeps absent actor signals explicit', contents.screen.includes('IDENTIDAD NO REPORTADA') && contents.screen.includes('FACCIÓN NO REPORTADA') && contents.screen.includes('RAREZA NO REPORTADA') && contents.screen.includes('DAÑO NO REPORTADO') && contents.screen.includes('NO REPORTADO') && !contents.screen.includes('Unidad no disponible') && !contents.screen.includes('actor?.faction ?? side') && !contents.screen.includes("actor?.rarity ?? 'REGISTRO OFICIAL'")],
   ['turn events preserve explicit server signals', contents.screen.includes('UNIDAD ELIMINADA POR VENENO') && contents.screen.includes('VENENO CONFIRMADO') && contents.battlefield.includes('UNIDAD ELIMINADA POR VENENO') && contents.battlefield.includes('VENENO CONFIRMADO') && contents.battlefield.includes('EVENTO NO REPORTADO') && !contents.screen.includes('Unidad derrotada') && !contents.battlefield.includes('TARGET LOCK')],
  ['turn detail preserves confirmed zero damage and live counts', contents.screen.includes('damage === null ?') && contents.screen.includes('numberSignal(turn.alive_a') && contents.screen.includes('numberSignal(turn.alive_b') && !contents.screen.includes("const damage = typeof turn.damage === 'number' ? turn.damage : 0")],
  ['formation preview preserves empty slot states', contents.formation.includes('POSICIÓN VACÍA') && contents.formation.includes('RESERVA VACÍA') && !contents.formation.includes('>Sin unidad<')],
  ...
  ['result preserves neutral and absent MMR states', contents.screen.includes('const mmrChange = typeof result.elo_change === \'number\' ? result.elo_change : null') && contents.screen.includes('mmrChange < 0 ? colors.danger : colors.accent') && contents.screen.includes('MMR NO REPORTADO') && contents.screen.includes('SIN MMR')],
  ['result keeps absent identity, match and turn signals explicit', contents.screen.includes('JUGADOR NO REPORTADO') && contents.screen.includes('RIVAL NO REPORTADO') && contents.screen.includes('MATCH NO REPORTADO') && contents.screen.includes('TURNOS NO REPORTADOS')],
  ['battle never invents an MMR reference or turn count', contents.screen.includes('playerMmr === null') && contents.screen.includes('totalTurns === null ?') && contents.screen.includes('typeof battleResult.total_turns === \'number\'') && !contents.screen.includes('playerMmr ?? 1000') && !contents.screen.includes('result.total_turns ?? result.turns?.length ?? 0')],
   ['battlefield keeps absent total turns as an absent signal', contents.screen.includes('const renderTurnIndex = totalTurns === null ? 0') && contents.screen.includes('turnIndex={renderTurnIndex}') && contents.screen.includes('totalTurns={totalTurns}') && contents.battlefield.includes('totalTurns: number | null') && contents.battlefield.includes('totalTurns !== null && totalTurns > 0') && !contents.screen.includes('const renderTotalTurns = totalTurns ?? 0')],
  ['rank card preserves absent record, MMR and shield states', contents.screen.includes('RÉCORD NO REPORTADO') && contents.screen.includes('MMR NO REPORTADO') && contents.screen.includes('shields === null ?') && contents.screen.includes('NO REPORTADO') && !contents.screen.includes('const shields = rank?.shields ?? 0')],
   ['rank card preserves the live season signal', contents.screen.includes('const seasonLabel = loading') && contents.screen.includes('rank?.season_id?.trim()') && contents.screen.includes('TEMPORADA EN ESPERA') && contents.screen.includes('TEMPORADA NO REPORTADA') && contents.screen.includes('TEMPORADA ACTIVA') && !contents.screen.includes('>TEMPORADA ACTIVA · RANGO PvP</Text>')],
