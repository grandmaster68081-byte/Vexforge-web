...
const assertions = [
  ...
  ['store keeps missing balance and card identity signals explicit', contents.screen.includes('BALANCE NO REPORTADO') && contents.screen.includes('function storeText') && contents.screen.includes('NOMBRE DE CARTA NO REPORTADO') && contents.screen.includes('RAREZA NO REPORTADA') && !contents.screen.includes("card.rarity ?? '—'")],
  ...
  ['store dates distinguish missing and invalid timestamps', contents.screen.includes('function formatStoreDate') && contents.screen.includes('FECHA NO REPORTADA') && contents.screen.includes('FECHA NO VÁLIDA') && contents.screen.includes('Number.isFinite(timestamp)') && contents.screen.includes('formatStoreDate(order.created_at)') && contents.screen.includes('formatStoreDate(boost.expires_at)') && !contents.screen.includes('new Date(order.created_at).toLocaleDateString') && !contents.screen.includes('new Date(boost.expires_at).toLocaleDateString')],
