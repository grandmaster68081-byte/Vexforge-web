...
const homeScene = await readFile('mobile/assets/images/home-reference-scene.png');
const homeSceneIsOfficialReference =
  homeScene.readUInt32BE(0) === 0x89504e47 &&
  homeScene.readUInt32BE(16) === 1080 &&
  homeScene.readUInt32BE(20) === 2340 &&
  homeScene[25] === 2;
...
const assertions = [
  ...
  ['Home uses the official 1080x2340 scene artwork', homeSceneIsOfficialReference && screen.includes('CANONICAL_BACKGROUNDS.home') && screen.includes('homeSceneSource') && screen.includes('styles.heroSceneReference')],
  ['Home composes native live identity and scene layers', screen.includes('styles.heroIdentityBackdrop') && screen.includes('identityCard?.image_url') && screen.includes('sceneAtmosphereStyle') && screen.includes('sceneMistStyle') && screen.includes('styles.heroSceneViewport')],
  ['Home scene has bounded ambient life without invented gameplay', screen.includes('sceneAtmosphereStyle') && screen.includes('sceneMistStyle') && screen.includes('pointerEvents="none"') && screen.includes('resizeMode="cover"') && screen.includes('heroParallaxStyle') && screen.includes('onLoad={() => setHomeSceneState')],
  ['Home does not mount a screenshot UI or hotspot overlay', !screen.includes('const HOTSPOTS') && !screen.includes('home-reference-') && screen.includes('testID="home-scene-state"')],
  ...
  ['Home keeps missing numeric signals contextual', screen.includes("function formatNumber(value: number | null | undefined, fallback = 'NO REPORTADO')") && screen.includes('function formatMetric') && screen.includes('function formatPair') && screen.includes('function formatEventProgress') && screen.includes("'XP NO REPORTADO'") && screen.includes("'MMR NO REPORTADO'") && screen.includes("'SIN POSICIONES PUBLICADAS'") && screen.includes('const progression =') && screen.includes('PROGRESIÓN EN ESPERA') && screen.includes('{progression ? <ProgressRail') && !screen.includes('value ?? 0') && !screen.includes("return '—';")],
  ['Home keeps event timing honest', screen.includes('const endsAtMs = new Date(endsAt).getTime()') && screen.includes('Number.isFinite(endsAtMs)') && screen.includes("'FECHA NO REPORTADA'") && !screen.includes('new Date(endsAt).getTime() - Date.now()')],
  ['Home keeps missing featured card codes explicit', screen.includes('function cardCodeLabel') && screen.includes('CÓDIGO NO REPORTADO') && !screen.includes("activeCard?.code ?? '—'")],
