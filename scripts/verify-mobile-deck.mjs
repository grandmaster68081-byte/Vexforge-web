...
const assertions = [
  ...
  ['deck is a programmatic surface without a forge skin', contents.screen.includes('testID="deck-programmatic-surface"') && contents.screen.includes('<DeckPreviewCard') && !contents.screen.includes('deck-forge-scene.jpg') && !contents.screen.includes('decks-reference-scene.png')],
  ['deck keeps the measured frame for responsive programmatic layout', contents.screen.includes('useMeasuredCanonicalFrame') && contents.screen.includes('onReferenceRootLayout') && contents.screen.includes('onLayout={onReferenceRootLayout}') && contents.screen.includes('referenceScene') && contents.screen.includes('marginTop: insets.top')],
  ...
  ['deck keeps the programmatic controls wired', contents.screen.includes('pointerEvents="box-none"') && contents.screen.includes('deckSlotHit') && contents.screen.includes('deck-refresh-visible')],
  ...
  ['deck keeps empty and missing summary signals explicit', contents.screen.includes("hasSavedDeck ? savedSummary.cardCount : 'SIN MAZO ACTIVO'") && contents.screen.includes("hasSavedDeck ? savedSummary.power : 'PODER NO REPORTADO'") && contents.screen.includes("hasSavedDeck ? savedSummary.factionLabel : 'FACCIONES NO REPORTADAS'") && contents.screen.includes("summary.championName ? `CAMPEÓN · ${summary.championName}` : 'CAMPEÓN NO REPORTADO'") && !contents.screen.includes('summary?.cardCount ?? 0')],
];
