...
**How to apply:** Keep the token out of URLs, command output, files in the repository, and logs; verify the remote branch after pushing. GitHub REST accepts Bearer/token headers, while Git HTTPS needs Basic auth as `x-access-token`; use an ephemeral askpass or header.