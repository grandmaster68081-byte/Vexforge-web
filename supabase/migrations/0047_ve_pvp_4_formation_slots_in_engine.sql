-- VE-PVP-04 — FORMACION VISIBLE EN EL MOTOR (CAMPEON / VANGUARDIA / CENTINELA)
--
-- Contexto: el tablero de batalla no podia representar las tres posiciones
-- canonicas porque `vexforge_battle_resolve` devolvia `is_champion` tal cual
-- venia de `player_decks` (false cuando el mazo no marca campeon) y nunca
-- exponia el slot que el motor YA resuelve internamente.
--
-- Esta migracion NO cambia ninguna regla de combate: no toca stats, dano,
-- keywords, ELO, XP, recompensas, reserva ni la bonificacion de faccion.
-- Solo publica en el JSON de salida:
--   * `slot` por unidad en final_units:
--     'champion' | 'vanguard' | 'sentinel' | 'reserve' | 'fallen'
--   * `is_champion` con el campeon realmente resuelto (incluido el fallback)
--   * `id` y `slot` de atacante y defensor en cada turno
--   * la unidad que entra desde la reserva hereda el slot; la caida => 'fallen'
--
-- APLICADA en produccion el 2026-09-12 y verificada con la cuenta QA real.
--
...
ALTER FUNCTION public.vexforge_battle_resolve(uuid, uuid, text) OWNER TO postgres;
