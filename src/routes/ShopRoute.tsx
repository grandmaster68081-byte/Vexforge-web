...
function date(iso?:string){ return iso ? new Date(iso).toLocaleString("es-MX",{day:"2-digit",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"}) : ""; }
    ...
    return <div style={{display:"grid",gap:6}}>
      {boosts.map(b=><div key={b.id} style={{background:"#0d1a0d",border:"1px solid #3dc96b44",borderRadius:8,padding:"8px 12px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div><div style={{color:C.green,fontWeight:700,fontSize:12,display:"flex",alignItems:"center",gap:6}}><ForgeIcon name="energy" size={12} />{BOOST_LABEL[b.boost_type]||b.boost_type}</div><div style={{color:C.muted,fontSize:10}}>×{b.multiplier} XP</div></div>
      ...
      {consumables.map(c=><div key={c.id} style={{background:"#1a0e08",border:"1px solid #f9731644",borderRadius:8,padding:"8px 12px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{color:"#f97316",fontWeight:700,fontSize:12,display:"flex",alignItems:"center",gap:6}}><ForgeIcon name="key" size={12} />{CONSUMABLE_LABEL[c.item_key]||c.item_key}</div>
...
function ItemCard({item,onBuy}:{item:ShopItem;onBuy:(item:ShopItem)=>void}) {
  const cat=CATEGORY_META[item.category]??{label:item.category,color:C.muted,icon:"shop" as ForgeIconName};
  ...
  return <div style={{background:C.bg1,border:"1px solid "+C.b1,borderRadius:12,padding:"16px 14px",display:"flex",flexDirection:"column",gap:10}}>
    <div style={{display:"flex",alignItems:"center",gap:10}}><div style={{color:cat.color,display:"grid",placeItems:"center"}}><ForgeIcon name={shopItemIcon(item)} size={28} /></div><div style={{flex:1}}><div style={{color:C.main,fontWeight:700,fontSize:14}}>{item.name}</div><div style={{color:cat.color,fontSize:10,fontWeight:700,background:cat.color+"18",border:"1px solid "+cat.color+"33",borderRadius:4,padding:"1px 6px",display:"inline-block",marginTop:3}}>{cat.label}</div></div></div>
...
export function ShopRoute(){
  ...
  const handleBuy=useCallback(async(item:ShopItem)=>{setOrderLoading(true);const reference="shop-"+item.item_key+"-"+(globalThis.crypto?.randomUUID?.()??String(Date.now()));const {data,error}=await supabase.rpc("vexforge_create_shop_order",{p_item_key:item.item_key,p_client_reference:reference,p_payment_reference:null,p_tx_hash:null,p_payer_wallet_address:null});setOrderLoading(false);if(error||!data?.ok){addToast("error",data?.reason??error?.message??"No se pudo crear la orden.");return;}setOrder(data as ShopOrder);setTxHash("");setPayerWallet("");loadOrders();},[addToast,loadOrders]);
  async function submitPayment(){if(!order?.order_id||!txHash.trim()||!payerWallet.trim())return;setOrderLoading(true);const {data,error}=await supabase.rpc("vexforge_submit_shop_order_payment",{p_order_id:order.order_id,p_tx_hash:txHash.trim(),p_payer_wallet_address:payerWallet.trim(),p_payment_reference:null});setOrderLoading(false);if(error||!data?.ok){addToast("error",data?.reason??error?.message??"No se pudo registrar el pago.");return;}setOrder({...order,...data,tx_hash:txHash.trim(),payer_wallet_address:payerWallet.trim(),payment_submitted:true});addToast("success","Pago registrado. Queda pendiente de confirmación administrativa.");loadOrders();}
  ...
  return <div style={{minHeight:"100vh",background:C.bg0,padding:"24px 16px",fontFamily:"system-ui,sans-serif",maxWidth:760,margin:"0 auto"}}>
    ...
    <div style={{marginTop:16,padding:"10px 14px",borderRadius:9,background:"#0d1a0d",border:"1px solid #3dc96b33",color:"#3dc96b",fontSize:11,display:"flex",alignItems:"center",gap:6}}><ForgeIcon name="check" size={12} />Todos los productos tienen fulfilment canónico activo. Las órdenes aprobadas se entregan automáticamente.</div>
