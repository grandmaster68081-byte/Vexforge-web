...
export function ShopRoute(){
  ...
  const handleBuy=useCallback(async(item:ShopItem)=>{setOrderLoading(true);const reference="shop-"+item.item_key+"-"+(globalThis.crypto?.randomUUID?.()??String(Date.now()));const {data,error}=await supabase.rpc("vexforge_create_shop_order",{p_item_key:item.item_key,p_client_reference:reference,p_payment_reference:null,p_tx_hash:null,p_payer_wallet_address:null});setOrderLoading(false);if(error||!data?.ok){addToast(data?.reason??error?.message??"No se pudo crear la orden.","error");return;}setOrder(data as ShopOrder);setTxHash("");setPayerWallet("");loadOrders();},[addToast,loadOrders]);
