...
function getDPSTier(dps: number): { label: string; color: string; icon: ForgeIconName } {
  if (dps >= 2000) return { label: 'LEYENDA',  color: '#ffd700', icon: 'spark' };
  if (dps >= 1200) return { label: 'MAESTRO',  color: '#e8b84b', icon: 'crown' };
  if (dps >= 600)  return { label: 'FORJADOR', color: '#a855f7', icon: 'energy' };
  if (dps >= 200)  return { label: 'APRENDIZ', color: '#4a9eff', icon: 'shield' };
  return { label: 'RECLUTA', color: '#8b8b9e', icon: 'attack' };
}
  ...
  return (
    <div onClick={onClick} className={auraClass} style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        ...
        <span style={{ color: "#e8b84b", fontSize: 9 }}><ForgeIcon name="energy" size={10} style={{ verticalAlign: "middle", marginRight: 3 }} />{card.power}</span>
      </div>
  ...
  return (
    <div style={{ width: 220, flexShrink: 0, position: "sticky", top: 16 }}>
        ...
        {factionList.length > 0 && (
          <div style={{ borderTop: "1px solid #1a1a2e", paddingTop: 10, marginTop: 8 }}>
            ...
            {Object.keys(factions).length > 2 && (
              <div style={{ color: "#e3573f", fontSize: 10, marginTop: 4 }}><ForgeIcon name="warning" size={11} style={{ verticalAlign: "middle", marginRight: 4 }} />Máx. 2 facciones</div>
            )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        <button onClick={onValidate} style={{ padding: "9px", borderRadius: 10, border: "1px solid #e8b84b44", background: "transparent", color: "#e8b84b", fontWeight: 700, fontSize: 12, cursor: "pointer" }}><ForgeIcon name="target" size={12} style={{ verticalAlign: "middle", marginRight: 5 }} />Validar</button>
        <button onClick={onSave} disabled={saving || !isValid} style={{ padding: "9px", borderRadius: 10, border: "none", fontFamily: "Cinzel,serif", fontWeight: 800, fontSize: 13, cursor: isValid && !saving ? "pointer" : "not-allowed", background: isValid ? "linear-gradient(135deg,#e8b84b,#c9901f)" : "#1a1a2e", color: isValid ? "#0a0a12" : "#444" }}>
          {saving ? "Guardando…" : <><ForgeIcon name="attack" size={13} style={{ verticalAlign: "middle", marginRight: 5 }} />Guardar Mazo</>}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      <div style={{ position: "relative", marginBottom: 10 }}>
        <span style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: "#5a5a7a", pointerEvents: "none" }}><ForgeIcon name="target" size={13} /></span>
        <input
          type="text"
  ...
  return (
    <main style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 16px" }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <h1 style={{ fontFamily: "Cinzel,serif", color: "#e8e8f0", fontSize: 26, margin: "0 0 4px" }}><ForgeIcon name="attack" size={24} style={{ verticalAlign: "middle", marginRight: 8 }} />Constructor de Mazos</h1>
      ...
      <div style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
        <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                  <div style={{ fontFamily: "Cinzel,serif", fontSize: 12, color: "#e8b84b", letterSpacing: "0.08em" }}>
                    <ForgeIcon name="energy" size={14} style={{ verticalAlign: "middle", marginRight: 6 }} />DECK POWER SCORE
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <ForgeIcon name={tier.icon} size={18} />
                ...
                <div style={{ fontSize: 10, color: "#7a7a9a", marginBottom: 6, fontFamily: "Rajdhani,sans-serif", letterSpacing: "0.08em" }}>
                  <ForgeIcon name="crown" size={13} style={{ verticalAlign: "middle", marginRight: 5 }} />DESIGNAR CAMPEÓN (Legendary · Mythic)
                </div>
                      ...
                      .map(c => {
                        ...
                        return (
                          <button key={c.card_id} onClick={() => setChampionId(isChamp ? null : c.card_id)} style={{
                            ...
                            {isChamp && <ForgeIcon name="crown" size={12} style={{ verticalAlign: "middle", marginRight: 3 }} />}{c.name}
                ...
                {champion && (
                  <div style={{ marginTop: 8, fontSize: 10, color: "#6a6a8a", fontFamily: "Rajdhani,sans-serif" }}>
                    <ForgeIcon name="crown" size={12} style={{ verticalAlign: "middle", marginRight: 4 }} /><span style={{ color: RARITY_COLOR[champion.rarity] }}>{champion.name}</span> —
          ...
          {myCards.length === 0 ? (
            <EmptyState icon={<ForgeIcon name="cards" size={36} />} title="Sin cartas" description="Aún no tienes cartas. Abre packs desde la tienda para empezar a construir tu mazo." />
          ) : filtered.length === 0 ? (
            <EmptyState icon={<ForgeIcon name="target" size={36} />} title="Sin resultados" description="Ninguna carta coincide con el filtro. Prueba con otros criterios." />
          ) : (
