 ...
 * backing table(s) for 'fusion' is service_role-only. There is no public or
 ...
 */
export async function getFusion(): Promise<DomainResult<never>> {
  return {
    status: "blocked_no_path",
    data: null,
    reason: "No public/authenticated RLS policy exists for 'fusion'. Needs a backend decision, not a frontend fix.",
  };
}
