...
{
    /// <summary>
    /// Strict bounded pool. Active and inactive capacity are independent.
    /// </summary>
    public sealed class VexforgeCardPool : IDisposable
    {
            ...
            int maxInactiveInstances)
        {
            ...
            if (maxActiveInstances <= 0) throw new ArgumentOutOfRangeException(nameof(maxActiveInstances));
            if (maxInactiveInstances < 0) throw new ArgumentOutOfRangeException(nameof(maxInactiveInstances));
        ...
        public VexforgeCardView Rent(
            ...
            CardArtMode artMode = CardArtMode.FullCardArtwork)
        {
            var view = Rent();
            if (view == null) return null;

            view.Bind(card, ownership, selected, activeInBattle, locked, resolver, artMode);
            return view;
        }
        ...
        public VexforgeCardView Rent()
        {
            ...
            VexforgeCardView view = inactive.Count > 0 ? inactive.Dequeue() : factory();
            if (view == null) return null;
