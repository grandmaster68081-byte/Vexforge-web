...
- Cambios: `src/routes/DepositRoute.tsx` (carga condicionada a `authed`, orden de estados corregido) y `src/domains/season/repository.ts` (`getMyPendingSeasonPassOrder` exige sesión viva antes de llamar a `vexforge_get_my_shop_orders`, que producía otro 401 anónimo en `/season-pass`). Sin cambios de esquema, Storage, manifiesto ni arte.
- Evidencia: `npm run verify:all` verde (typecheck, build + verify-build, ui-identity 188 archivos/0 violaciones, identity-data 9 tablas/274 filas/0 violaciones, boss-art 15+15, card-art 127, surface-art 29/18/11, residual-art 51/32/29, manifest 218 HEAD/0 rotas, assets 21/21). Comprobación local con `vite preview` y Playwright anónimo en `/deposit`, `/season-pass`, `/shop`, `/withdrawal` y `/profile`: 0 respuestas >= 400, 0 errores de consola y aviso "ACCESO REQUERIDO" visible en depósito.
