...
- **BUILD:** el único workflow de compilación es `.github/workflows/vexforge-unity-android-github.yml`, con `workflow_dispatch` y modos internos para normal, diagnóstico, baseline, inventory, shard y final; checkpoint es un artefacto encadenado, no otro workflow. No se lanzó ningún workflow en esta sesión.
- **INCREMENTAL FLOW:** el workflow ahora exige shards secuenciales, valida cobertura completa sin duplicados/huecos antes de `final`, conserva `ShaderCache` y `PlayerDataCache` con `ScriptsOnlyCache.yaml` y `DataBuildDirtyInfo.json`, y genera un único APK final sin filtro ni límite de 35.000 variantes.
...
- **RELEASE GATE:** commit `ded78720d1d3f97280d5702166d44e18415beb8f` compilado por workflow `35238357900`; terminó `success`, publicó `vexforge-android-build-249` y el APK contiene `assets/index.android.bundle`.
- **STATUS:** `LEGACY_ARTIFACT_IDENTIFIED / APK_RELEASE_PUBLISHED / BUNDLE_VERIFIED`.
...
- **ENTORNO ACTIVO:** aplicación Android en `mobile/**`; la web permanece congelada.
- **AUTORIDAD:** fila `ACTIVE` de `public.vexforge_official_documents / vexforge_home_world_system_protocol_v3`, V2.3, SHA-256 `cd83250a5f63fe72b5be237bfdf1bb54a1f935927a5fea95136190b837a165ba`.
- **SCREEN / DOMAIN:** `ECONOMY` / Iron Treasury; encabezado y selector táctil de Cartera, Mercado, Depósitos, Retiros y Referidos.
- **BLOCK:** centralizar geometría, escala, tipografía y presión ya existentes sin cambiar balances, fórmulas, datos, RPCs, acciones ni estados honestos.
- **WHAT CHANGED:** `VISUAL_TOKENS.economy` define shell, encabezado, regreso, rail y sección; `economy.tsx` los consume.
- **BEHAVIOR PRESERVED:** mismas cinco secciones, rutas, `testID`, accesibilidad, refresh, selección y opacidad de presión `0.72`.
- **DATA PRESERVED:** no se añadieron saldos, precios, rewards, contratos, solicitudes, RPCs, RLS, economía, gameplay, assets ni cambios en la web congelada.
- **MOTION / PERFORMANCE:** no se añadió movimiento, listener ni trabajo en segundo plano; sólo se registraron roles estáticos existentes.
- **GUARD:** `node scripts/verify-mobile-economy.mjs`, `node scripts/verify-mobile-visual-system.mjs`, ambos `node --check` y `git diff --check`.
- **RELEASE GATE:** no se inicia workflow Android, no se compila APK y no se publica release; QA visual/táctil física queda pendiente.
- **STATUS:** `IMPLEMENTED_UNVERIFIED`.
- **PERSISTENCIA:** publicar código, guardia, documento T2V y esta continuidad juntos en `main` mediante la API REST HTTPS oficial de GitHub con `[skip ci]`, sin `git push`.
- **NEXT BLOCK:** continuar con otra pieza T2V explícita, sin tocar el motor de juego, los contratos de Supabase, la web congelada ni el tutorial.
...
- **ENTORNO ACTIVO:** aplicación Android en `mobile/**`; la web permanece congelada.
- **AUTORIDAD:** fila ACTIVE `public.vexforge_official_documents / vexforge_home_world_system_protocol_v3`, V2.2, con SHA-256 `828a077428987b2534e3733204ac721ef3073eb7dadf9530a6f46877dd63c503`.
- **SCREEN / DOMAIN:** Battlefield Android: arte ausente, reserva, estados, fallbacks de carta e identidad de formación.
- **BLOCK:** eliminar tamaños locales de iconos sin cambiar el event log, unidades, roles, legalidad, resultado, replay ni contratos de touch.
- **WHAT CHANGED:** `VISUAL_TOKENS.battlefield.icons` define los roles `artMissing`, `reserveHeader`, `reserveFallback`, `stateHeader` e `identity`; `ForgeBattlefield` los consume.
- **BEHAVIOR PRESERVED:** mismos tamaños efectivos `18`, `13`, `12`, `13` y `14`; mismos iconos, estados honestos, accesibilidad, datos vivos y assets autorizados.
- **DATA PRESERVED:** no se añadieron solicitudes, estado, rutas, RPCs, RLS, economía, gameplay, settlement, assets ni cambios en la web.
...
- **ENTORNO ACTIVO:** aplicación Android en `mobile/**`; la web permanece congelada.
- **AUTORIDAD:** fila ACTIVE `public.vexforge_official_documents / vexforge_home_world_system_protocol_v3`, V2.2, con SHA-256 `828a077428987b2534e3733204ac721ef3073eb7dadf9530a6f46877dd63c503`.
- **SCREEN / DOMAIN:** sigilos de la cabecera compartida de FOJA, ARENA, ARCHIVO, FORJA y LEGADO.
- **BLOCK:** eliminar el tamaño literal del icono del sigilo sin modificar identidad, iconos oficiales, color, copy, navegación ni estados.
- **WHAT CHANGED:** `VISUAL_TOKENS.domainHeader.sigilIconSize` define el tamaño compartido y `DomainHeader` lo consume.
- **BEHAVIOR PRESERVED:** mismo tamaño efectivo `17`, sigilos canónicos, tonos de dominio, accesibilidad, navegación y datos vivos.
- **DATA PRESERVED:** no se añadieron solicitudes, estado, rutas, RPCs, RLS, economía, gameplay, assets ni cambios en la web.
...
- **ENTORNO ACTIVO:** aplicación Android en `mobile/**`; la web permanece congelada.
- **AUTORIDAD:** fila ACTIVE `public.vexforge_official_documents / vexforge_home_world_system_protocol_v3`, V2.2, con SHA-256 `828a077428987b2534e3733204ac721ef3073eb7dadf9530a6f46877dd63c503`.
- **SCREEN / DOMAIN:** cabecera transversal de FOJA, ARENA, ARCHIVO, FORJA y LEGADO; estado de conexión inicial del shell de navegación.
- **BLOCK:** eliminar los últimos pesos tipográficos aislados en superficies compartidas sin cambiar copy, identidad, navegación, sesión ni estados de dominio.
- **WHAT CHANGED:** `VISUAL_TOKENS.domainHeader.place.fontWeight`, `VISUAL_TOKENS.domainHeader.title.fontWeight` y `VISUAL_TOKENS.navigation.authLoading.labelWeight` ahora son roles explícitos; `DomainHeader` y `TabLayout` los consumen.
- **BEHAVIOR PRESERVED:** mismos pesos efectivos `900`, `800` y `700`, copy, sigilos, propósito, rutas, redirects, Auth, Supabase, haptics y accesibilidad.
- **DATA PRESERVED:** no se añadieron solicitudes, estado, rutas, RPCs, RLS, economía, gameplay, assets ni cambios en la web.
- **MOTION / PERFORMANCE:** no se añadió movimiento, listener, render de datos ni timing; sólo se centralizaron valores estáticos de presentación.
- **GUARD:** `node scripts/verify-mobile-visual-system.mjs`, `node --check scripts/verify-mobile-visual-system.mjs` y `git diff --check` pasan.
- **RELEASE GATE:** no se inició workflow Android, no se compiló APK y no se publicó release; QA visual/táctil física queda pendiente.
- **STATUS:** `IMPLEMENTED_UNVERIFIED`.
- **PERSISTENCIA:** publicar código, guardia y esta continuidad juntos en `main` mediante la API REST HTTPS oficial de GitHub con `[skip ci]`, sin `git push`.
- **NEXT BLOCK:** continuar con otra pieza T2V explícita, sin tocar el motor de juego, los contratos de Supabase, la web congelada ni el tutorial.
...
- **ENTORNO ACTIVO:** aplicación Android en `mobile/**`; la web permanece congelada.
- **AUTORIDAD:** fila ACTIVE `public.vexforge_official_documents / vexforge_home_world_system_protocol_v3`, V2.2, con SHA-256 `828a077428987b2534e3733204ac721ef3073eb7dadf9530a6f46877dd63c503`.
- **BASELINE:** `main` en `d30704594477b4b05e2c5c95688ca4107e88c082`, sin cambios de código móvil.
- **DOCUMENTO:** se añadió `docs/VE-T0-ANDROID-ASSET-REGISTRY.json` como registro estructurado derivado de `src/lib/assetManifest.ts`, `mobile/assets/images/` y `docs/VEXFORGE-ASSET-ATLAS.md`.
- **COBERTURA:** 80 entradas: 13 archivos locales con dimensiones y SHA-256 medidos, 22 rutas Storage `VERIFIED_ASSET` y 45 rutas `RESERVED_SURFACE_ART`/`RESERVED_RESIDUAL_ART` con hash y dimensiones `NOT_MEASURED`.
- **REGLA DE PROCEDENCIA:** ningún asset reservado, `AVAILABLE_UNASSIGNED` o `REVIEW_REQUIRED` se promueve a consumidor; las rutas remotas no reciben hash, dimensiones ni compatibilidad inventados.
- **ALCANCE:** no se tocó `mobile/**`, Supabase, Auth, RLS, RPCs, economía, gameplay, assets físicos, rutas ni la web congelada; el registro sólo documenta lo existente.
- **STATUS:** `PREPARED / EVIDENCE_REQUIRED`; T0 continúa abierto y `PROFILE_HISTORY_STATE_GAP` sigue `BLOCKED`.
- **NO APK:** no se inició workflow Android, no se compiló APK, no se instaló build y no se publicó release; el cambio documental usa `[skip ci]`.
- **PERSISTENCIA:** registry y continuidad se publican juntos en `main` mediante GitHub REST/Data API sobre HTTPS, sin `git push` ni credenciales en URLs.
- **NEXT BLOCK:** otra unidad documental independiente o, sólo con autorización explícita, el gate físico de APK/QA; no promover `VERIFIED`, `TIER1_READY`, `SUPPORTED` u `OPERATIONAL`.
...
- **BRANCH:** `main`.
- **SCREEN / DOMAIN:** `WORLD ATLAS` / tarjeta de World Raid.
- **BLOCK:** centralizar la presentación del raid sin tocar participación,
  sesión, RPCs, rewards, estado, datos vivos ni autoridad del servidor.
- **WHAT CHANGED:** `VISUAL_TOKENS.worldRaid` registra shell, stripe, identidad,
  dificultad, métricas, código y acción; `RaidCard` consume esos roles y la
  guardia T2V exige su presencia.
- **BEHAVIOR PRESERVED:** mismos `world-raid-*`, `onJoin`, `onContribute`,
  `joined`, `busy`, acciones oficiales, mensajes de validación y controles de
  accesibilidad.
- **DATA PRESERVED:** no se añadieron solicitudes, estado, raids, rewards,
  Supabase, Auth, RPCs, economía, gameplay, assets ni cambios en la web
  congelada.
- **MOTION / PERFORMANCE:** no se añadió movimiento, listener, render de
  datos, timing ni trabajo en segundo plano; sólo se registraron roles
...
## 2026-09-15 — VE-MOB-T2V — WORLD BOSS CARD TOKENS / IMPLEMENTED_UNVERIFIED

- **BRANCH:** `main`.
- **SCREEN / DOMAIN:** `WORLD ATLAS` / tarjeta de World Boss.
- **BLOCK:** centralizar la presentación de la tarjeta sin tocar daño,
  encounters, rewards, lore, battle route ni resolución autoritativa.
- **WHAT CHANGED:** `VISUAL_TOKENS.worldBoss` registra arte, identidad, tier,
  body, señales de HP/recompensa y acción; `BossCard` consume esos roles y la
  guardia T2V exige su presencia.
- **BEHAVIOR PRESERVED:** mismos `world-boss-*`, `onBattle`, `PREPARAR
  BATALLA`, datos vivos, fallback honesto de arte, colores dinámicos,
  accesibilidad y valores visuales efectivos.
- **DATA PRESERVED:** no se añadieron solicitudes, estado, bosses, encounters,
  daño, rewards, lore, Supabase, Auth, RPCs, economía, gameplay, assets ni
  cambios en la web congelada.
- **MOTION / PERFORMANCE:** no se añadió movimiento, listener, render de
  datos, timing ni trabajo en segundo plano; sólo se registraron roles
  estáticos de presentación.
- **AUTHORITY:** fila ACTIVE de
...
## 2026-09-15 — VE-MOB-T2V — WORLD ATLAS HEADER AND TABS / IMPLEMENTED_UNVERIFIED

- **BRANCH:** `main`.
- **SCREEN / DOMAIN:** `WORLD ATLAS` / cabecera, actualización y pestañas.
- **BLOCK:** centralizar la presentación de la entrada al mundo sin cambiar
  paneles, datos vivos, rutas, acciones ni contratos.
- **WHAT CHANGED:** `VISUAL_TOKENS.worldHeader` registra la geometría del
  encabezado, sello, refresh, copy, tabs y texto; `WorldHeader` consume esos
  roles y la guardia T2V exige su presencia.
- **BEHAVIOR PRESERVED:** mismos `world-refresh`, cinco destinos de panel,
  `onRefresh`, selección de pestaña, estados de carga, copy, colores,
  accesibilidad y tamaño efectivo de iconos.
- **DATA PRESERVED:** no se añadieron solicitudes, estado, bosses, raids,
  lore, temporada, rankings, Supabase, Auth, RPCs, economía, gameplay, assets
  ni cambios en la web congelada.
- **MOTION / PERFORMANCE:** no se añadió movimiento, listener, render de
  datos, timing ni trabajo en segundo plano; sólo se movieron roles estáticos
  de presentación al registro visual.
- **AUTHORITY:** fila ACTIVE de
  `public.vexforge_official_documents / vexforge_home_world_system_protocol_v3`,
  confirmada por HTTPS antes de la implementación.
- **GUARD:** `node scripts/verify-mobile-world.mjs`,
...
- **BRANCH:** `main`.
- **SCREEN / DOMAIN:** `PROFILE` / aviso de sincronización y estado de carga.
- **BLOCK:** extraer la presentación del feedback transversal sin cambiar
  mensajes, sesión, errores vivos ni la fuente de datos.
- **WHAT CHANGED:** `VISUAL_TOKENS.profileFeedback` define el aviso de error,
  su icono y texto, además de la separación y etiqueta del estado de carga;
  `profile.tsx` consume esos roles.
- **BEHAVIOR PRESERVED:** mismos `syncError`, `detailsError`, `testID`,
  `accessibilityRole`, `ActivityIndicator`, copy, insets, tamaños y colores
  resueltos por el estado actual.
- **DATA PRESERVED:** no se añadieron solicitudes, estado, rutas, perfil,
  ranking, Supabase, Auth, RPCs, economía, gameplay, assets ni cambios en la
  web.
- **MOTION / PERFORMANCE:** no se añadió movimiento, listener, render de datos
...
- **BLOCK:** completar la jerarquía de pesos tipográficos del campo de batalla
  sin cambiar la semántica de combate ni los datos vivos.
- **WHAT CHANGED:** eyebrow, título, turno, identidad, unidades, reserva,
  carril, daño y resultado consumen pesos de
  `VISUAL_TOKENS.battlefield.typography`.
- **BEHAVIOR PRESERVED:** mismos pesos efectivos `900`, `800` y `700`,
  tamaños, tracking, line-height, formación, HP, eventos, replay y resultado.
- **DATA PRESERVED:** no se añadieron solicitudes, estado, rutas, gameplay,
  Supabase, Auth, RPCs, assets ni cambios en la web.
- **MOTION / PERFORMANCE:** no se añadió movimiento, listener, render de datos
...
- **BRANCH:** `main`.
- **SCREEN / DOMAIN:** `FORJA` / `ForgeFormationPreview`.
- **BLOCK:** registrar dos colores estáticos de presentación sin convertirlos
  en datos ni alterar la formación autoritativa.
- **WHAT CHANGED:** el placeholder de arte y el borde de feedback consumen
  `VISUAL_TOKENS.formation.cardArt.placeholderBackground` y
  `VISUAL_TOKENS.formation.feedback.borderColor`.
- **BEHAVIOR PRESERVED:** misma apariencia, formación, estados loading,
  empty, error, retry, accesibilidad, radios, bordes y zonas de interacción.
- **DATA PRESERVED:** no se añadieron solicitudes, estado, rutas, gameplay,
  Supabase, Auth, RPCs, assets ni cambios en la web.
- **MOTION / PERFORMANCE:** no se añadió movimiento, listener, render de datos
  ni timing; sólo se centralizaron dos valores de presentación existentes.
- **GUARD:** `node scripts/verify-mobile-visual-system.mjs`,
  `node --check scripts/verify-mobile-visual-system.mjs` y `git diff --check`.
- **RELEASE GATE:** no se inició workflow Android, no se compiló APK y no se
  publicó release; QA visual/táctil queda pendiente de autorización.
...
## 2026-09-14 — VE-MOB-T2V — FORMATION PREVIEW BORDER TOKENS / IMPLEMENTED_UNVERIFIED

- **BRANCH:** `main`.
- **SCREEN / DOMAIN:** `FORJA` / `ForgeFormationPreview`.
- **BLOCK:** unificar los bordes de la vista de formación con el token visual
  compartido sin cambiar la formación autoritativa ni sus estados.
- **WHAT CHANGED:** la raíz, el sigil, las tarjetas, el feedback y el retry
  consumen `VISUAL_TOKENS.border.standard` en lugar de literales locales.
- **BEHAVIOR PRESERVED:** datos de `DeckSlot`, roles de Vanguardia,
  Campeón, Centinela y Reserva, loading, empty, error, retry, accesibilidad,
  colores dinámicos, radios y zonas de interacción.
- **DATA PRESERVED:** no se añadieron solicitudes, estado, rutas, gameplay,
  Supabase, Auth, RPCs, assets ni cambios en la web.
- **MOTION / PERFORMANCE:** no se añadió movimiento, listener, render de
  datos ni cambio de timing; sólo se eliminó duplicación visual estática.
- **GUARD:** `node scripts/verify-mobile-visual-system.mjs`,
  `node --check scripts/verify-mobile-visual-system.mjs` y `git diff --check`.
- **RELEASE GATE:** no se inició workflow Android, no se compiló APK y no se
  publicó release; QA visual/táctil queda pendiente de autorización.
...
- **RELEASE GATE:** no se inició workflow Android, no se compiló APK y no se
  publicó release; QA visual/táctil queda pendiente de autorización.
- **STATUS:** `IMPLEMENTED_UNVERIFIED`.
- **COMMIT:** `31e23517380c7f5dc530c50bdd81f1e6dda13357` publicado en `main`;
...
- **RELEASE GATE:** no se inició workflow Android, no se compiló APK y no se
  publicó release; QA visual/táctil queda pendiente de autorización.
- **STATUS:** `IMPLEMENTED_UNVERIFIED`.
- **COMMIT:** `3ed3fc0ca5c2b9da93668c623004e246e3387984` publicado en `main`;
  no se generó workflow APK por `[skip ci]`.
- **PERSISTENCIA:** publicar código, guarda T2V y esta continuidad juntos en
  `main` mediante la API REST HTTPS oficial de GitHub con `[skip ci]`, sin
  `git push`.
...
- **RELEASE GATE:** no se inició workflow Android, no se compiló APK y no se
  publicó release; QA visual/táctil queda pendiente de autorización.
- **STATUS:** `IMPLEMENTED_UNVERIFIED`.
- **COMMIT:** `0881eba84f45890ed5e2b45dfc11e982680762a7` publicado en `main`;
  no se generó workflow APK por `[skip ci]`.
- **PERSISTENCIA:** publicar código, guarda T2V y esta continuidad juntos en
  `main` mediante la API REST HTTPS oficial de GitHub con `[skip ci]`, sin
  `git push`.
...
- **SCREEN / DOMAIN:** `ARENA` / `ForgeBattlefield`.
- **BLOCK:** centralizar la jerarquía tipográfica del campo de batalla sin
  alterar la semántica de combate ni los datos vivos.
- **WHAT CHANGED:** `VISUAL_TOKENS.battlefield.typography` define tamaños,
  interlineados, tracking y offsets de encabezado, turno, identidad, roles,
  cartas, reserva, carril de confrontación y resultado; `ForgeBattlefield`
  consume esos roles.
- **DATA PRESERVED:** `BattleTurn`, `BattleUnit`, HP, daño, eventos, roles,
  reservas, estados de caída, lado desconocido, resultado y labels honestos del
  servidor.
- **MOTION / PERFORMANCE:** no se añadió movimiento, solicitud, listener ni
  render de datos; las animaciones y gates de reduced-motion permanecen iguales.
- **GUARD:** `node scripts/verify-mobile-visual-system.mjs` y
  `git diff --check`.
- **RELEASE GATE:** no se inició workflow Android, no se compiló APK y no se
  publicó release; QA visual/táctil queda pendiente de autorización.
...
- **EVIDENCIA DE PUBLICACIÓN:** commit `7343cc87fe481ae4856bf1eaa2bf0a50626e7ee8` publicado en `main`; `VEXFORGE_PROTOCOL_V2.md` y la fila ACTIVE comparten MD5 `1eddad7cdb4e5f809f8ce36ae21370a4`; SHA-256 de la imagen `d122f6af31439b048ced3d3ffbf1a1be14847c9fef97774ed11dd056ba15aa94`.
...
- **BRANCH:** `main`.
- **SCREEN / DOMAIN:** Arena / `ForgeBattlefield` y sus sockets de formación, reserva e identidad.
- **BLOCK:** decimotercera pieza ejecutable de `T2V — VISUAL SYSTEM TOKENS + SHARED SCENE PRIMITIVES`.
- **WHAT CHANGED:** el battlefield consume `VISUAL_TOKENS.battlefield` para contenedor, identidad, separación de formación, tarjetas de unidad, arte, reserva y carril de confrontación.
- **DATA PRESERVED:** event log, turnos, HP, daño, formación, Campeón/Vanguardia/Centinela/Reserva, ganador, replay, RPC oficial, Auth, RLS, economía, accesibilidad, reduced-motion, assets oficiales y web congelada.
- **AUTHORITY:** Android continúa renderizando el contrato vivo; no se alteran targeting, iniciativa, daño, protección, slot count, reglas, resultados ni settlement.
- **ASSETS USED:** se mantienen únicamente arte y señales oficiales ya recibidos por `BattleUnit`; no se añade fallback diegético.
- **MOTION_ADDED:** ninguno; la animación existente sigue gobernada por eventos y `reducedMotion`.
- **PERFORMANCE_NOTES:** tokens estáticos resueltos localmente; sin solicitudes, listeners, estado ni renders de datos adicionales.
- **GATES:** `node scripts/verify-mobile-visual-system.mjs`, `node --check scripts/verify-mobile-visual-system.mjs` y `git diff --check`; typecheck móvil, workflow APK y QA visual/táctil siguen pendientes y no se inician.
- **VISUAL_DELTA:** la escena de combate comparte ahora geometría explícita para sockets, cartas de formación, reserva y carril sin reinterpretar el estado autoritativo.
- **VISUAL_DEBT_REMOVED:** literales geométricos duplicados en la primitiva de battlefield.
- **VISUAL_DEBT_ADDED:** ninguna.
- **STATUS:** `IMPLEMENTED_UNVERIFIED`.
- **PERSISTENCIA:** publicar código, guarda de visual system y continuidad juntos en `main` mediante la API REST HTTPS oficial de GitHub con `[skip ci]`; no compilar APK ni publicar release.
- **NEXT_BLOCK:** continuar con otra pieza T2V explícita sin tocar el motor, los contratos de Supabase, la web congelada ni el tutorial.
...
- **BRANCH:** `main`.
- **SCREEN / DOMAIN:** World / `WORLD → RAIDS EN CURSO`.
- **BLOCK:** evitar tarjetas sin nombre o código cuando el raid llega con identidad vacía.
- **WHAT CHANGED:** nombres ausentes muestran `NOMBRE DE RAID NO REPORTADO`; códigos ausentes muestran `CÓDIGO DE RAID NO REPORTADO`; cada señal conserva su propio valor cuando existe.
- **LIVE DATA CHECK:** `raid_runs` continúa accesible con la cuenta QA mediante lectura HTTPS; no se ejecutaron mutaciones.
- **DATA PRESERVED:** raids, nombres, códigos, regiones, estados, dificultad, límites, multiplicadores, fechas, participantes, recompensas, RPCs oficiales, Auth, RLS, navegación, accesibilidad, reduced-motion y web congelada.
- **ASSETS USED:** ninguno nuevo.
- **MOTION_ADDED:** ninguno.
...
- **CORE_SCORE:** pendiente de QA visual/táctil en dispositivo.
- **VISUAL_DEBT_ADDED:** ninguna.
- **VISUAL_DEBT_REMOVED:** nombre de raid vacío y código de raid vacío como identidad implícita.
- **STATUS:** `IMPLEMENTED_UNVERIFIED`.
- **PERSISTENCIA:** código, documento y continuidad se publicarán juntos en `main` mediante la API REST HTTPS oficial de GitHub con `[skip ci]`; sin APK ni release Android nuevo.
- **NEXT_BLOCK:** continuar con otra unidad Android atómica del protocolo sin tocar la web congelada ni iniciar APK.
...
- **STATUS:** `IMPLEMENTED_UNVERIFIED`.
- **PERSISTENCIA:** código y continuidad publicados en `main` mediante la API REST HTTPS oficial de GitHub en `29e7d1945a26e2930c51c6694098d4c6451ce4d5` con `[skip ci]`; sin APK ni release Android nuevo.
- **NEXT_BLOCK:** continuar con otra unidad Android atómica del protocolo sin tocar la web congelada ni iniciar APK.
...
- **PERSISTENCIA:** código publicado en `main` mediante la API Git Data HTTPS oficial de GitHub en `6da558cf7fe0d47dd8e5640a5a03c3c248788738` con `[skip ci]`; no se generará release Android.
- **SIGUIENTE BLOQUE:** continuar con otra mejora Android atómica verificable, sin tocar la web congelada ni iniciar compilación hasta autorización del operador.
...
- **BLOCK:** lectura de estados de depósitos, retiros y referidos.
- **WHAT CHANGED:** los estados conocidos del contrato vivo ahora se muestran como etiquetas legibles (`PENDIENTE`, `REVISIÓN PENDIENTE`, `APROBADO`, `COMPLETADO`, `ENTREGADO`, `RECHAZADO`, `FALLIDO`, `CANCELADO`). Los estados desconocidos conservan su valor mediante `ESTADO: ...` y los ausentes muestran `ESTADO NO REPORTADO`.
- **VISUAL DELTA:** la cámara económica deja de exponer códigos crudos sin perder información ni convertir estados no reconocidos en una semántica inventada.
- **DATA PRESERVED:** estados, contadores, RPCs económicas, ledger, mercado, tesorería, depósitos, retiros, referidos, Auth, RLS, assets, navegación y web congelada.
- **ASSETS USED:** ninguno nuevo; se mantienen los assets oficiales registrados.
- **MOTION_ADDED:** ninguno.
- **PERFORMANCE_NOTES:** traducción local de strings ya recibidos; sin red adicional ni animación.
...
- **PERSISTENCIA:** código publicado en `main` mediante la API Git Data HTTPS oficial de GitHub en `a40be5994ac8dc194b72b01abeee711490837bf8` con `[skip ci]`; no se generará release Android.
- **SIGUIENTE BLOQUE:** continuar con otra mejora Android atómica verificable, sin tocar la web congelada ni iniciar compilación hasta autorización del operador.
...
- **QUÉ CAMBIÓ:** la cabecera propia del panel fue sustituida por `DomainHeader` con `domain="forja"`, conservando el refresh visible en el slot de acción compartido.
- **FIDELIDAD DE DATOS:** el estado de la cabecera usa exclusivamente loading de Colección/Mazos, `syncState`, `deckError` y el resumen de slots oficiales ya cargado; muestra cartas/facciones reales sólo cuando existe mazo guardado.
- **ESTADOS:** carga comunica `SINCRONIZANDO MAZOS`; error/offline comunica `SIN SEÑAL · TOCA PARA REINTENTAR`; mazo guardado comunica cantidad y facciones; sin mazo conserva el propósito de Forja.
- **CONTRATOS PRESERVADOS:** colección, validación, guardado, límites, rutas, frame medido, controles programáticos, accesibilidad y navegación inferior. No se tocaron Supabase, RPCs, Auth, economía, combate ni la web congelada.
...
- **PERSISTENCIA:** cambio publicado en `main` mediante la API REST HTTPS de GitHub en el commit `b944250403f4987a1f92632faab1d8bb464df538` con `[skip ci]`, por lo que no se inició el workflow Android.
- **EVIDENCIA LOCAL:** `verify:mobile-battle` pasa 27/27. El typecheck del clon sigue bloqueado porque faltan las dependencias Expo (`expo/tsconfig.base` y módulos nativos); no se presenta ese bloqueo del entorno como un fallo del delta.
- **ESTADO HONESTO:** `IMPLEMENTED_UNVERIFIED`; la QA visual/táctil en APK y el release correlativo siguen pendientes de autorización.
- **SIGUIENTE BLOQUE:** continuar sólo con otro delta Android atómico que mejore feedback o fidelidad sin reabrir contratos.
...
## 2026-09-13 — VE-MOB-8-REWARDS — REWARD ACTION PRESS DEPTH / IMPLEMENTED_UNVERIFIED

- Se continuó el rollout Tier 1 Android exclusivamente en `mobile/app/missions.tsx`: reclamar una quest y comenzar una misión ahora responden al toque con profundidad física coherente.
- Se conservaron quests y misiones reales, reclamación/ejecución por RPC, VEX/XP/T-VEX, energía, cooldowns, estados de carga/error/vacío/resultado, telemetría, testID, accesibilidad y reduced-motion. No se modificaron Supabase, RLS, RPCs, Auth, economía, combate, assets ni la web congelada.
- Commit Android publicado en `main`: `1d1643fc6833a299791c2fcfda713928fe6d68b6`. Usa `[skip ci]` por instrucción del operador para no iniciar compilación ni release. El estado permanece `IMPLEMENTED_UNVERIFIED`.
...
- La segunda pasada completó la integración de la arquitectura: los seis dominios dejaron de depender de tarjetas grandes como elemento dominante, los cuatro accesos de la Cámara se convirtieron en nodos verticales iconográficos, y los marcos de evento, órdenes y portales adoptaron geometría de umbral con trazas, esquinas y gateways direccionales. Los textos funcionales se mantienen como HUD, estado y etiqueta diegética; no son la estructura principal de interacción.
...
---
## 2026-09-12 — VE-MOB-3-HOME — FINAL TCG SCENE COMPOSITION / IMPLEMENTED_UNVERIFIED

- Se completó el pase final de composición exclusivamente en `mobile/app/(tabs)/index.tsx`, tomando el video de referencia y la directiva TCG premium como criterio visual.
- El Home deja de presentar el Nexus como un panel contenedor único: la carta oficial gana presencia en el hero y en el frente activo, el frente/evento funciona como una escena compartida, los dominios se leen como constelación y Forja/Economía/actividad/clasificación quedan como rieles de operación con separación visual.
...
---
## 2026-09-13 — VE-MOB-3-HOME — REDUCED MOTION PARALLAX CLOSURE / IMPLEMENTED_UNVERIFIED

- Se cerró un micro-bloque de accesibilidad y polish exclusivamente en `mobile/app/(tabs)/index.tsx`: el parallax del artwork del hero, la identidad en primer plano y la atmósfera del Continuum ahora quedan estáticos cuando `reduced-motion` está activo.
- Se conservaron la escena TCG del Nexus, los datos vivos de Supabase, la identidad y carta canónicas, las rutas, los `testID`, la accesibilidad, los estados loading/partial/error/vacío, refresh, haptics y la animación ambiental normal.
...
- Al presionar, cada objeto reduce intensidad y desciende dentro de su plano sin alterar navegación, expansión de carta, haptics, accesibilidad ni los estados activo/en espera del evento.
- Se conservaron artwork y datos canónicos, parallax acotado, `reduced-motion`, rutas, `testID`, Supabase/GameContext y todas las superficies fuera del Home.
- No se inicia workflow, no se compila APK ni se genera release por la instrucción vigente. Estado honesto: `IMPLEMENTED_UNVERIFIED`.
- Siguiente microbloque: continuar sólo con una mejora concreta de continuidad o interacción, sin rehacer la escena consolidada.
...
- Commit remoto de implementación: `3f498966a7c10300d7ce05caf57737608bb847d2`.
- Siguiente microbloque: continuar con otra mejora Android atómica de fidelidad o interacción, sin tocar la web congelada ni iniciar APK sin autorización.
...
- **IMPLEMENTACIÓN:** `CANONICAL_BACKGROUNDS.pvp` ya alimenta un viewport vivo dentro de `ForgeBattlefield`: imagen de escena con drift/escala ambiental limitada, parallax de scroll nativo acotado a 8 dp, iluminación cyan/ember, viñeta, HUD nativo, formaciones reales, Campeón/Vanguardia/Centinela, Reserva, carril de evento, daño, resultado y señales honestas de Mano/Command.
- **SAFE AREA / LEGIBILIDAD / RENDIMIENTO:** `battle.tsx` posee explícitamente insets verticales y horizontales, indicadores de scroll y ajuste de contenido desactivado para evitar doble compensación; el viewport usa gradientes de contraste, paneles opacos, tamaños mínimos de lectura y líneas limitadas. El scroll y las animaciones ambientales usan `useNativeDriver`, `scrollEventThrottle=16`, `extrapolate=clamp`, sólo dos glows animados y sin blur/partículas.
