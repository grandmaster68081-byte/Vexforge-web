...
- **Evidencia pública:** `build-manifest.json` público informa `sourceCommit=5bf3e6765f35b2109d84812456a705c4f4126446` y `sourceBranch=main`. HTTP 200 con cache-busting en `/`, `/withdrawal`, `/pvp`, `/season-pass`, `/leaderboard`, `/cards`, `/battle`, `/tutorial`, `/economy` y `/profile`. Hashes SHA-256 locales y públicos coincidentes en `HomeRoute-94fKGnGf.js`, `WithdrawalRoute-Darja2YU.js`, `SeasonPassRoute-CxGdhQeI.js` y `LeaderboardRoute-7-vk3DH-.js`; el bundle público de `HomeRoute` no contiene caracteres de dibujo de caja. Navegación real en Chromium (1280x1800) sobre `/`, `/leaderboard`, `/season-pass`, `/withdrawal`, `/cards` y `/pvp`: 0 errores de consola y 0 `pageerror`; los encabezados renderizan las reglas CSS del eyebrow.
- **Siguiente acción verificable:** revisión visual responsive en móvil real con foco visible y `prefers-reduced-motion`, y QA autenticado del aviso de retiro y del historial de partidas (sigue `BLOCKED` sin sesión normal de jugador).
...
## 2026-08-17 — VE-SEC-2-DEFINER-EXPOSURE-TRIAGE — IMPLEMENTED_UNVERIFIED

- **Tipo de sesión:** AUDITORÍA DE SUPABASE + IMPLEMENTACIÓN (unidad de seguridad autoritativa). **Unidad siguiente declarada por VE-SEC-1.**
- **Fuente canónica:** catálogo vivo del proyecto `rscuzqnfccqvltkdcdny` (`pg_proc`, `has_function_privilege`, `pg_get_functiondef`), asesor de seguridad de Supabase, código real de `main` y `VEXFORGE_PROTOCOL_V2.md`.
- **Estado inicial:** deuda registrada por VE-SEC-1 — 124 funciones `SECURITY DEFINER` en `public`, **92 ejecutables por `anon`** y 112 por `authenticated`. **Estado actual:** `IMPLEMENTED_UNVERIFIED`. **Baseline:** commit `55c804a`.
- **Auditoría (triaje función por función, sin revoke masivo):**
  - **Hallazgo crítico:** `public.grant_achievement(uuid, text)` es `SECURITY DEFINER` sin ninguna comprobación de identidad: recibe un `player_id` arbitrario e **inserta logro + acuña `vex_ingame` y XP** en `player_wallet` / `player_progress`. Era ejecutable con la clave publicable (`anon`) → acuñación de economía por cualquier portador de la clave. `fn_check_and_grant_achievements(uuid)` comparte el motor y también aceptaba jugadores ajenos.
  - **Superficie administrativa expuesta a `anon`** (13 funciones): `vexforge_is_control_admin`, `vexforge_admin_get_overview/players/ledger/deposits/withdrawals/shop_orders`, `vexforge_approve_deposit`, `vexforge_reject_deposit`, `vexforge_approve_shop_order`, `vexforge_reject_shop_order`, `admin_reject_withdrawal`, `vexforge_grant_relic`. Todas verifican internamente `vexforge_is_control_admin()`, pero la exposición al rol público es innecesaria y facilita el sondeo.
- **Verificación previa a implementar:** único llamador de `grant_achievement` en la base es `fn_check_and_grant_achievements`, y de ésta `check_my_achievements()` (que resuelve el jugador por `auth.uid()`); ambas son `SECURITY DEFINER` con owner `postgres`, por lo que **no dependen de estos grants**. En el código, el cliente sólo invoca `check_my_achievements` (`src/domains/achievements/repository.ts`); las 13 RPCs administrativas se llaman exclusivamente desde `src/domains/admin/*` en rutas de admin autenticadas.
- **Cambio:** migración idempotente `supabase/migrations/0012_ve_sec2_definer_exposure_triage.sql`:
  1. `REVOKE ALL ... FROM PUBLIC, anon, authenticated` sobre `grant_achievement` y `fn_check_and_grant_achievements` (+ `GRANT EXECUTE` explícito a `service_role`).
  2. `REVOKE ALL ... FROM PUBLIC, anon` sobre las 13 funciones administrativas, con `GRANT EXECUTE` explícito a `authenticated` y `service_role`.
  - Nota técnica registrada: PostgreSQL concede `EXECUTE` a `PUBLIC` por defecto, así que revocar sólo a `anon` no surte efecto; la primera versión de la migración se corrigió tras comprobarlo en el catálogo vivo.
- **Contrato de datos:** sin cambios de esquema, tablas, RPCs (cuerpo), triggers, RLS, Storage, economía ni UI. Sólo privilegios de ejecución.
- **Verificaciones ejecutadas:** migración aplicada vía Management API sin error; recuento de definer ejecutables por `anon` **92 → 77**; matriz de privilegios confirmada (`grant_achievement`/`fn_check_and_grant_achievements`: anon=false, authenticated=false; las 13 admin: anon=false, authenticated=true); prueba REST real con la clave publicable → `grant_achievement` y `fn_check_and_grant_achievements` responden **HTTP 401 `permission denied`**, `vexforge_admin_get_overview` **401**, y la ruta legítima `check_my_achievements` sigue respondiendo **204**; asesor de seguridad: **0 ERROR**, WARN 205 → 188, INFO 4; `npm ci --ignore-scripts`, `npx tsc --noEmit -p tsconfig.app.json` sin errores, `npm run build` correcto, `npm run verify:assets` 17/17.
- **Deuda registrada:** quedan **77** funciones `SECURITY DEFINER` ejecutables por `anon` (mayoría son funciones de trigger `trg_fn_*` y de progresión del jugador que resuelven identidad por `auth.uid()`); pendiente el siguiente lote de triaje sobre las que aceptan `player_id` explícito (`ensure_player_row`, `initialize_player_full`, `vexforge_assign_starter_deck`, `grant_starter_relics`, `apply_ranked_result`, `refresh_player_energy`, `vexf_consume_energy`). Sigue pendiente: RLS activa sin políticas en `public.vexforge_system_config`, `econ_sim.events` y `econ_sim.players`; protección de contraseñas filtradas desactivada en Auth (consola del propietario); higiene documental de tablas `vexforge_*`.
...
## 2026-08-17 — VE-SEC-5-RLS-NO-POLICY-DEFAULT-DENY — IMPLEMENTED_UNVERIFIED

- **Tipo de sesión:** AUDITORÍA DE SUPABASE + IMPLEMENTACIÓN (unidad de seguridad autoritativa). **Unidad siguiente declarada por VE-SEC-4** (cierre por defecto de RLS sin políticas con recorte de grants).
- **Fuente canónica:** catálogo vivo del proyecto `rscuzqnfccqvltkdcdny` (`pg_class`, `pg_policy`, `relacl`, `has_table_privilege`, `has_schema_privilege`), REST real con la clave publicable y código real de `main`. **Baseline:** commit `1bc50ab`.
- **Verificación de cierre de VE-SEC-4:** ya registrada en su cierre operativo (`sourceCommit=973aa11`, rutas críticas 200, navegación sin errores). VE-SEC-4 permanece **VERIFIED**.
- **Auditoría:** barrido de tablas con `relrowsecurity=true` y **0 políticas** en `public` y `econ_sim`. Resultado: 4 tablas — `public.vexforge_system_config`, `public.vexforge_icon_legacy` (hallazgo nuevo, no listado en la deuda previa), `econ_sim.events`, `econ_sim.players`. Las dos de `public` tenían `relacl` con **todos los privilegios para `anon` y `authenticated`**: la prueba REST previa con la clave publicable devolvía **200 `[]`** (RLS niega filas, pero la tabla y su forma quedaban expuestas). Sin ningún consumidor cliente: `vexforge_icon_legacy` es tabla de auditoría de iconos leída por funciones definer propiedad de `postgres` (migraciones `0008`/`0009`), `vexforge_system_config` es configuración interna, y las `econ_sim.*` son de simulación económica interna.
- **Cambio:** migración idempotente y transaccional `supabase/migrations/0015_ve_sec5_rls_no_policy_default_deny.sql`: `REVOKE ALL ... FROM PUBLIC, anon, authenticated` en las 4 tablas y en el esquema `econ_sim`; `GRANT` sólo a `service_role` (+ `USAGE` de esquema); `ENABLE ROW LEVEL SECURITY` reafirmado. **No se crean políticas**: el estado canónico declarado de estas tablas es DENEGACIÓN POR DEFECTO.
- **Contrato de datos:** sin cambios de esquema, columnas, datos, cuerpos de funciones, triggers, políticas, Storage, economía ni UI. Sólo privilegios.
- **Alcance no modificado:** autenticación, energía, wallet, MMR, temporadas, recompensas, misiones, navegación y resultados autoritativos. No se usó `service_role` para suplantar jugadores ni para fabricar QA.
- **Verificaciones ejecutadas:** migración aplicada vía Management API sin error; catálogo posterior → las 4 tablas con `relacl = {postgres, service_role}`, `has_table_privilege(anon|authenticated,'select') = false`, `relrowsecurity=true`, `pol=0`; `econ_sim` con `nspacl = {postgres, service_role}` y `has_schema_privilege('anon','usage') = false`; REST real con clave publicable → `vexforge_system_config` y `vexforge_icon_legacy` pasan de **200 `[]`** a **401 `42501 permission denied`**; sin regresión en lecturas públicas conservadas (`get_home_stats` y `get_leaderboard` siguen **200**); `npm ci --ignore-scripts`, `npx tsc --noEmit -p tsconfig.app.json` sin errores y `npm run verify:build` correcto.
- **Deuda registrada:** protección de contraseñas filtradas desactivada en Auth (consola del propietario, fuera de alcance del agente); higiene documental de tablas internas `vexforge_*`; definer de progresión resueltas por `auth.uid()` bajo `authenticated` (triaje de menor riesgo); glyphs Unicode en datos internos de motores (`aiBattleEngine`, `missionEncounterEngine`, `dailyChallenge`, `forgeFormation`); artes duplicados del bucket sin decisión canónica.
- **Bloqueos:** QA autenticado de superficies del jugador sigue `BLOCKED` sin sesión de jugador autorizada.
